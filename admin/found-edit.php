    ...
    if($row['exist'] > 0){
        ...
        else
        {
           //Inform founder 
            $docstatus = 1;
            $docid = $row['docid'];
            $poster1=$_SESSION['user_id'];
            $poster2=$row['docposter'];
            $docdeclarer=$poster2;
            //check why is returns an hour before
            ...
            $update = "UPDATE document SET docstatus = 1, docdeclarer=:declarer, docfound=:founded,docuser='Admin' WHERE docid = :doc";
            $found="INSERT INTO notification(sendernot,receivernot,notsender,notreceiver,notdoc,notuser) VALUES(:content1,:content2,:poster1,:poster2,:notdoc,'Admin')";
...
$sql="UPDATE document SET docowner=:owner,doctype=:type,docno=:no,docplace=:place WHERE docid=:doc";
   ...
   {
$sql = "INSERT INTO document (docowner,doctype,docno,docplace,docposter,docdeclarer,docfound,docstate,docstatus,docuser) VALUES (:owner,:type,:no,:place,:poster,:docdeclarer,:docfounded,'1',:status,'Admin')";
   }
    $stmt = $pdo->prepare($sql);
...
else
{
    ...
    $actsql = "INSERT INTO activitylog(actdesc,actuser,actuserid) VALUES('New found document record','Admin',$actuserid)";
    $pdo->exec($actsql);
    }
    else
    {
    $actsql = "INSERT INTO activitylog(actdesc,actuser,actuserid) VALUES('Updated found document record','Admin',$actuserid)";
    $pdo->exec($actsql);          
 ?>
