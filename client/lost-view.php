                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="product-status-wrap">
                            ...
                            <div class="add-product">
                                <a href="lost-edit.php" data-toggle="tooltip" title="Add new">Add Lost</a>
                            </div>
                            <table>
                                <tr>
                                    <th>Image</th>
...
$list = "SELECT * FROM client INNER JOIN document ON document.docposter=client.clid INNER JOIN documenttype ON documenttype.dtypeid=document.doctype WHERE  document.docposter =$clientid AND document.docstate='0' order by document.docregdate DESC";
...
while ($row=$result->fetch(PDO::FETCH_ASSOC)) {
    # code...
                                    ...
                                    ?>
                                    <tr>
                                    <td><img src="avatar/doc.jpg" alt="" /></td>
   ...
   if ($row['docdeclarer'] == '0') {
           ...
           $mode = '';
                                ...
                                ?>
                                        <button class="ds-setting"><i>In progress</i></button>
...
{
$list2 = "SELECT * FROM users WHERE userid=$founder";
$result2 = $pdo->query($list2);
while ($row2=$result2->fetch()) {
echo $row2['ufname']; 
}  
...
if (date('Y-m-d')===date('Y-m-d',$thatt)) {
   # code...
     echo "<br><i>Today ".date('H:i',$thatt)."</i>";
...
else
{
    echo " <br><i>".date("d M Y",$thatt)."</i>";
                                     ...
                                     ?></td>
                                    <td>
                                        <button onclick="location.href='lost-edit.php?lost_id=<?php  echo $row['docid'];  ?>'" data-toggle="tooltip" title="Edit"  <?php echo $mode; ?> class="pd-setting-ed"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>
...
function deleteDoc(Id){
    ...
    if(iAnswer){
        window.location = 'lost-view.php?del_id=' + Id;
    }
  }
</script>
       <?php
